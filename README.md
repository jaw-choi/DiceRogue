...
## Map Scene 작업 후 확인하실 항목

- Unity에서 `MapScene`을 실행하신 뒤, 시작 위치가 좌하단이고 보스가 우상단에 보이는지 확인해 주세요.
- 시작 위치와 보스만 처음부터 보이고, 나머지 칸은 구름으로 가려져 있는지 확인해 주세요.
- 이동 가능한 칸만 클릭 가능하고, 새로 이동 가능해진 칸의 구름이 자연스럽게 페이드아웃되는지 확인해 주세요.
- `Assets/Resources/map.png`와 `Assets/Resources/map_icon.png`의 스프라이트 이름이 유지되는지 확인해 주세요.
- `map.png`는 `map_0`, `map_icon.png`는 `map_icon_0`부터 `map_icon_4`까지 유지되어야 합니다.
- 실제 플레이로 `Map -> Battle/Reward/Shop -> Map` 루프를 한 번 이상 돌려 보시고, 현재 위치 표시가 정상적으로 따라오는지 확인해 주세요.
- 맵 배치나 경로를 바꾸고 싶으시면 `Assets/Game/Scripts/Runtime/Systems/RunContentFactory.cs`의 `CreateGridMapNodes` 입력 시드를 수정해 주세요.
- 최종 씬 정리 단계에서는 기존에 씬에 직접 올려 두셨던 맵 이미지나 버튼 UI를 정리하셔도 됩니다.
- 현재 구현은 런타임으로 맵 UI를 다시 생성하는 방식입니다.
